...
public class EventClassifier {
    ...
    /**
     * Classifica evento como "green", "yellow" ou "red".
     * Usa combinação de type/action/time e campos auxiliares do Event.
     */
    public static String classify(Event e) {
        if (e == null) return "green";
        ...
        if ("focus".equals(type)) {
            if ("focus_lost".equals(action)) {
                ...
                if (e.getTime() > 0 && e.getTimestamp() > 0) {
                    ...
                    if (elapsed >= FOCUS_RED_MS) return "red";
                }
                // default: red (perda de foco é sempre um sinal forte)
                return "red";
            }
            if ("focus_gained".equals(action)) return "green";
        }

        // 2) ALT e combinações
        if ("keyboard".equals(type)) {
            if ("alt".equals(action)) return "yellow"; // alt sozinho -> suspeita
            if (action.contains("alt+tab") || action.contains("alt_tab") || action.contains("alttab"))
                return "red";
            if ("ctrl+c".equals(action) || "ctrl+v".equals(action) || action.contains("copy") || action.contains("paste")) {
                // copy/paste: suspicious; multiple occurrences -> escalate (precisa contar no redis)
                return "yellow";
            }
            // TAB by itself is normal
            if ("tab".equals(action) || "enter".equals(action) || action.length() == 1) return "green";
        }
        ...
        if ("pause".equals(type)) {
            // assume action contains duration in ms (optional), else use event.time difference
            try {
                long pauseMs = Long.parseLong(action.replaceAll("\\D", ""));
                if (pauseMs >= PAUSE_THRESHOLD_MS) return "yellow";
